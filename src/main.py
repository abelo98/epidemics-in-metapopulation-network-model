...
def calc_params_with_acc_infected_combine(est: estimator, acc_infected, guess_path):
    acc_infected_combine = data_operator.combine_infected_all_mcps(
        acc_infected)
    return est.get_params_estimation_combine_infected(
        acc_infected_combine, guess_path)


def calc_params_with_acc_infected_by_muncps(est: estimator, acc_infected, guess_path):
    return est.get_params_estimation_metamodel(
        acc_infected, guess_path)


def calc_params_by_munc_model(est: estimator, acc_infected, guess_path):
...
def main():
    ...
    guess_path = "src/mmodel/params_estimation/params_guess.json"
    # ydata = initialize()
