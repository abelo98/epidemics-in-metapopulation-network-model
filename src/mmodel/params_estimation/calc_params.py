...
def __get_params__(params, munc, popt):
    params_estimated = {}
    print(munc)
    print("params: ")
    print("")
...
def estimate_params(ydata: np.array, time: np.array, params: list, initial_v: dict, munc):
    ...
    return __get_params__(params, munc, popt)
...
def set_initial_values(model, infected):
...
def build_json_params(models_json, infected, params_to_estimate):
    output = []
    for model in models_json:
        initial_v, infected_by_munc, munc, time = set_initial_values(
            model, infected)
...
def get_params_estimation(nodes_params_json_path, infected, params_to_estiamte):
    models = read_json(nodes_params_json_path)
