...
def plot_est_and_especial_points(y_est):
    ...
    ranges = [(0, 100), (100, int(y_est.argmax())), (int(y_est.argmax()), int(y_est.argmax())),
...
def main():
    ...
    # params_est1 = 'tests/mmodel/havana_geo_connections_SAIR/estimation/parameters_estimated_pso_SAIR_Numba_GPU_d29_iter-12000.json'
    ...
    y_estimated_I_SIR = get_data_simulation(est2, False, days)['I']

    # est3 = estimator_test(network_path=network3,
    #                       params=params_est3, numba=False)
    # y_estimated = get_data_simulation(est, False, days)

    # y_estimated_I = y_estimated['I']
    # y_estimated_A = y_estimated['A']

    # label1 = 'I(t) SAIR metapoblaciones municipios colindantes conectados'
    # label2 = 'A(t) SAIR metapoblaciones municipios colindantes conectados'
    label3 = 'I(t) todos los municipios conectados'
    # label4 = 'I(t) SIR metapoblaciones todos los municipios conectados'
    label4 = 'datos reales'

    # run_test()
    compare_est_with_org(y_estimated_I_SIR, infected_all_combine)
    plot_multiple([y_estimated_I_SIR, infected_all_combine], [
                  label3, label4])
