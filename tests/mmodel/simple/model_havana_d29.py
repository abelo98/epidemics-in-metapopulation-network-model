...
def deriv(y, t, params):
	...
	result[1] =  -(params[5] * (y[13] + y[9]) * y[1]) / (y[21]) + 0.5 * y[0] - 0.5 * y[1]
	result[2] =  -(params[2] * (y[14] + y[10]) * y[2]) / (y[22]) + 0.5 * y[3] - 0.5 * y[2]
	result[3] =  -(params[5] * (y[15] + y[11]) * y[3]) / (y[23]) + 0.5 * y[2] - 0.5 * y[3]
	result[4] =  (params[2] * (y[12] + y[8]) * y[0]) / (y[20]) - params[0] * y[4] + 0.5 * y[5] - 0.5 * y[4]
	result[5] =  (params[5] * (y[13] + y[9]) * y[1]) / (y[21]) - params[3] * y[5] + 0.5 * y[4] - 0.5 * y[5]
	result[6] =  (params[2] * (y[14] + y[10]) * y[2]) / (y[22]) - params[0] * y[6] + 0.5 * y[7] - 0.5 * y[6]
	result[7] =  (params[5] * (y[15] + y[11]) * y[3]) / (y[23]) - params[3] * y[7] + 0.5 * y[6] - 0.5 * y[7]
